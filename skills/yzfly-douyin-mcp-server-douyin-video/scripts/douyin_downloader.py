...
环境变量:
- SILICONFLOW_API_KEY: 硅基流动 API 密钥 (用于文案提取功能)

配置文件:
- ../.env: Skill 目录下的 .env 文件，格式: SILICONFLOW_API_KEY=xxx
...
使用示例:
  ...
  # 提取文案并保存到文件 (需要 SILICONFLOW_API_KEY)
...
def load_api_key():
    """加载 SILICONFLOW_API_KEY，优先级: 环境变量 > ../.env 文件"""
    # 1. 先检查环境变量
    key = os.environ.get("SILICONFLOW_API_KEY")
    if key:
        return key.strip()

    # 2. 检查 skill 目录下的 .env 文件
    env_path = Path(__file__).parent.parent / ".env"
    if env_path.exists():
        try:
            txt = env_path.read_text(encoding="utf-8", errors="ignore")
            m = re.search(r"^\s*SILICONFLOW_API_KEY\s*=\s*(.+?)\s*$", txt, re.M)
            if m:
...
class DouyinProcessor:
    ...
    def extract_text_from_audio(self, audio_path: Path, show_progress: bool = True) -> str:
        ...
        if not self.api_key:
            raise ValueError("未设置 API 密钥，请设置环境变量 SILICONFLOW_API_KEY")
...
def extract_text(share_link: str, api_key: Optional[str] = None, output_dir: Optional[str] = None,
    ...
    if not api_key:
        raise ValueError("未设置 SILICONFLOW_API_KEY，请先获取硅基流动 API 密钥")
...
示例:
  ...
  # 提取文案并保存到文件 (需要设置 SILICONFLOW_API_KEY)
  python douyin_downloader.py --link "抖音分享链接" --action extract --output ./output
  ...
  python douyin_downloader.py --link "抖音分享链接" --action extract --output ./output --save-video
    ...
    parser.add_argument("--api-key", "-k", help="硅基流动 API 密钥 (也可通过 SILICONFLOW_API_KEY 环境变量设置)")
