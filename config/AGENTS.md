...
### 阅读配置文件和记忆：
1. 阅读 SOUL.md — 这是你的身份。并且soul.md会指引你阅读SOP_CONTENT.md 的场景索引，匹配场景后仅读取对应的 SOP_SCENE_x_xxx.md 文件。
...
阅读 `SOP_CONTENT.md` 中的场景匹配索引，判断当前消息属于哪个场景，然后**仅读取对应的场景文件**执行。

### Step 2：按场景执行
严格按照对应场景文件（`SOP_SCENE_x_xxx.md`）中的步骤执行，**不要跳步**。
...
**示例**：
```
收到链接 → 读取 SOP_CONTENT.md 索引 → 匹配到场景1 → 读取 SOP_SCENE_1_素材入库.md → 按步骤执行
```
不要请求许可。直接做。
