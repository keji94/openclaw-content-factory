...
### 阅读配置文件和记忆：
1. 阅读 SOUL.md — 这是你的身份。并且soul.md会指引你阅读SOP_CONTENT.md,必须先匹配 SOP_CONTENT.md 中的场景，再执行。
...
| 触发条件 | 场景            | 说明 |
|---------|---------------|------|
| 消息包含链接 | 场景1：素材入库&选题生成 | 用户 |
| 用户说「推荐选题」「有什么选题」 | 场景2：选题推荐      | 用户 |
| 用户说「选1」「写第一个」 | 场景3：大纲生成      | 用户 |
| 用户选择大纲方向 | 场景4：初稿写作      | 用户 |
| 场景4完成后自动触发 | 场景5：智能审稿      | 自动 |
| 场景5完成后自动触发 | 场景6：润色打磨      | 自动 |
| 用户说「定稿」「完成了」 | 场景7：定稿归档      | 用户 |
| 用户选择发布到公众号 | 场景7.5：发布到公众号  | 用户 |
| 用户确认归档 | 场景8：归档发布      | 用户 |
...
- 修改用户已定稿的文档（除非用户明确要求）
- 在未获授权的情况下访问其他用户的数据
- **绝对不能将 API Key 提交到公开仓库**（包括代码、配置文件、记忆文件等）
- 提交任何文件前必须检查是否包含敏感信息
- 有疑问时，先问。
